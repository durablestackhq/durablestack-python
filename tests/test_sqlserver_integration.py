...
from durablestack.sqlserver import SqlServerDurableStackOptions, create_durable_stack_sqlserver
...
@pytest.mark.skipif(_sqlserver_dsn() is None, reason="Set DURABLESTACK_TEST_SQLSERVER_DSN to run SQL Server tests")
async def test_sqlserver_enqueue_claim_and_succeed() -> None:
    dsn = _sqlserver_dsn()
    assert dsn is not None

    handle = await create_durable_stack_sqlserver(
        SqlServerDurableStackOptions(connection_string=dsn, table_prefix="ss_it_"),
    )
    store = handle.store
    try:
...
@pytest.mark.skipif(_sqlserver_dsn() is None, reason="Set DURABLESTACK_TEST_SQLSERVER_DSN to run SQL Server tests")
async def test_sqlserver_runtime_smoke_start_enqueue_stop() -> None:
    dsn = _sqlserver_dsn()
    assert dsn is not None

    handle = await create_durable_stack_sqlserver(
        SqlServerDurableStackOptions(connection_string=dsn, table_prefix="ss_rt_"),
        options=DurableStackOptions(
            worker_name="sqlserver-runtime-smoke",
...
@pytest.mark.skipif(_sqlserver_dsn() is None, reason="Set DURABLESTACK_TEST_SQLSERVER_DSN to run SQL Server tests")
async def test_sqlserver_recurring_slot_unique_under_contention() -> None:
    dsn = _sqlserver_dsn()
    assert dsn is not None

    handle = await create_durable_stack_sqlserver(
        SqlServerDurableStackOptions(connection_string=dsn, table_prefix="ss_rec_"),
    )
    store = handle.store
    try:
