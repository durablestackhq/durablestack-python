...
@pytest.mark.skipif(_postgres_dsn() is None, reason="Set DURABLESTACK_TEST_POSTGRES_DSN to run Postgres tests")
async def test_postgres_runtime_smoke_start_enqueue_stop() -> None:
    dsn = _postgres_dsn()
    assert dsn is not None

    handle = await create_durable_stack_postgres(
        PostgresDurableStackOptions(connection_string=dsn, table_prefix="py_rt_"),
        options=DurableStackOptions(
            worker_name="pg-runtime-smoke",
...
@pytest.mark.skipif(_postgres_dsn() is None, reason="Set DURABLESTACK_TEST_POSTGRES_DSN to run Postgres tests")
async def test_postgres_recurring_slot_unique_under_contention() -> None:
    dsn = _postgres_dsn()
    assert dsn is not None

    handle = await create_durable_stack_postgres(
        PostgresDurableStackOptions(connection_string=dsn, table_prefix="py_rec_"),
    )
    store = handle.store
    try:
